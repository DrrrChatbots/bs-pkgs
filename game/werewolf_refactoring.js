...
state night_seer {
  ...
  if !filter(players, (p, idx) => p.role == 2 && p.life).length then
    later (getRandom(10, 30) * 1000) going night_wolf
  else {
    asked = 0
    ...
    event dm (seer, cont) => {
      if !(seer in players) then
        drrr.print(me(T("beQuiet")(seer)))
      else if players[seer].role != 2 then
        drrr.dm(seer, T("notSeer"))
      else if !players[seer].life then
        drrr.dm(seer, T("deadSeer"))
      else if asked then
        drrr.dm(seer, T("oneAsk"))
      else {
        the = select(cont, names)
        if !the then
          drrr.dm(seer, T("noSuchPeople"))
        else {
...
state night_wolf {
  ...
  event dm (wolf, cont) => {
    ...
    if !(wolf in players) then
      drrr.print(me(T("beQuiet")(wolf)))
    else if players[wolf].role != 0 then
...
state night_witch {
  ...
  if !filter(players, (p, idx) => p.role == 3 && p.life).length then
    later (getRandom(10, 30) * 1000) going night_end
  else {
    poisoned = 0
    ...
    event dm (witch, cont) => {
      if !(witch in players) then
        drrr.print(me(T("beQuiet")(witch)))
      else if players[witch].role != 3 then
        drrr.dm(user, T("notWitch"))
      else if !players[witch].life then
        drrr.dm(user, T("deadWitch"))
      else if poisoned then
        drrr.dm(user, T("onePoiSav"))
      else {
        the = select(cont, names)
        if the == witch then
          ...
          drrr.dm(user, T("unkCmd"))
        else if !the.length then
          drrr.dm(user, T("noSuchPeople"))
        else if !players[the].life then
          drrr.dm(user, T("deadMan"))
        else {
